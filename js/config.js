...
export let config = {
	...
	min: {
		...
		intro: `在展示当前游戏模式武将胜率排行榜时，只在符合本配置条件的记录中筛选
			<br>此外如果您开启了［胜率代替权重］，那么内奸AI将优先采用符合本配置数值的数据作为武将权重计算`,
		init: '10',
		input: true,
		onblur: function (e) {
			let text = e.target,
	...
	qzCf: {
		...
		intro: '开启后，ai在计算卡牌效益时会将权重（若无则为1）与结果作积返回，以此鼓励ai优先“待遇”权重大的角色。会累乘下面的威胁度相关功能赋值，不推荐同时使用',
		init: false
	},
	chooseQz: {
	...
	applyCf: {
		name: '威胁度补充',
		intro: `〔按评级自动补充〕会在进入游戏时根据武将评级对没有添加威胁度的武将技能增加一定威胁度；
